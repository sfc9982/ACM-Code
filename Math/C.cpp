...
void solve() {
    cin >> a >> b >> c >> d >> e >> f;
    ll  xbnd = lstTrue(0, 4000000, [&](ll x) {
        return (4 * c * a - e * e) * x * x <= 4 * c * f;
    });
    ll  ybnd = lstTrue(0, 4000000, [&](ll x) {
        return (4 * c * a - e * e) * x * x <= 4 * a * f;
    });
    ...
    int ans = 0;
    F0R(i, N)
    ans = add(ans, mul(resv[i], mpow(i, 33)));
...
int main() {
    ...
    miv[0] = miv[1] = 1;
    FOR(i, 2, MX)
    miv[i] = mod - (long long) mod / i * miv[mod % i] % mod;
    while (T--) {
        solve();
    }
    return 0;
